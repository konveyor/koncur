	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, analysis *config.AnalysisConfig) {
				...
				if len(analysis.Rules) != 2 {
					t.Fatalf("Expected 2 rules, got %d", len(analysis.Rules))
				}

				// First rule
				comp0 := analysis.Rules[0].Git.GetCompents()
				if comp0 == nil {
					t.Error("Expected first rule to have Git components")
				} else {
					if comp0.URL != "https://github.com/konveyor/rulesets" {
						t.Errorf("First rule URL mismatch: %s", comp0.URL)
					}
					if comp0.Ref != "main" {
						t.Errorf("First rule ref mismatch: %s", comp0.Ref)
					}
					if comp0.Path != "java/cloud-readiness" {
						t.Errorf("First rule path mismatch: %s", comp0.Path)
					}
				}

				// Second rule
				comp1 := analysis.Rules[1].Git.GetCompents()
				if comp1 == nil {
					t.Error("Expected second rule to have Git components")
				} else {
					if comp1.URL != "https://github.com/konveyor/analyzer-lsp" {
						t.Errorf("Second rule URL mismatch: %s", comp1.URL)
					}
					if comp1.Ref != "v2.0" {
						t.Errorf("Second rule ref mismatch: %s", comp1.Ref)
					}
					if comp1.Path != "dotnet/migration" {
						t.Errorf("Second rule path mismatch: %s", comp1.Path)
					}
				}
			},
		},
